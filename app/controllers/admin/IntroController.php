...
class IntroController extends AdminController {
	...
	{
		// Grab all the pages
		$posts = Post::where('post_type', 'intro')->orderBy('created_at', 'DESC')->paginate(30);

		// Show the page
		return View::make('backend/intro/index', compact('posts'));
	}
	...
	public function getCreate()
	{
		...
		// Show the page
		return View::make('backend/intro/create', compact('categories'));
	}
	...
	public function postCreate()
	{
		...
		if($post->save())
		{
			...
			return Redirect::to("admin/intro/$post->id/edit")->with('success', Lang::get('admin/pages/message.create.success'));
		}

		// Redirect to the page create
		return Redirect::to('admin/intro/create')->with('error', Lang::get('admin/pages/message.create.error'));
	}
	...
	public function getEdit($postId = null)
	{
		...
		if (is_null($post = Post::find($postId)))
		{
			...
			return Redirect::to('admin/intro')->with('error', Lang::get('admin/pages/message.does_not_exist'));
		}
		...
		// Show the page
		return View::make('backend/intro/edit', compact('post', 'media', 'widgets'));
	}
	...
	public function postEdit($postId = null)
	{
		...
		if($post->save())
		{
			...
			// Redirect to the new page
			return Redirect::to("admin/intro/$postId/edit")->with('success', Lang::get('admin/pages/message.update.success'));
		}

		// Redirect to the pages management
		return Redirect::to("admin/intro/$postId/edit")->with('error', Lang::get('admin/pages/message.update.error'));
	}
	...
	public function getDelete($postId)
	{
		...
		{
			// Redirect to the pages management
			return Redirect::to('admin/intro')->with('error', Lang::get('admin/pages/message.not_found'));
		}
		...
		// Redirect to the pages management
		return Redirect::to('admin/intro')->with('success', Lang::get('admin/pages/message.delete.success'));
	}
}