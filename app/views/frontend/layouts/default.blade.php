...
<head>
	...
	@if(isset($page))
		@if(isset($media))
			<meta property="og:image" content="{{ asset($media->mpath . '/550x500/' . $media->mname) }}"/>
		@else
			<meta property="og:image" content="{{ asset('assets/images/640x295_noimage.jpg') }}"/>
		@endif
	@elseif(isset($post))
		<meta property="og:image" content="{{ asset($post->mpath . '/550x500/' . $post->mname) }}"/>
	@elseif(isset($product))
		<meta property="og:image" content="{{ asset($product->mpath . '/550x500/' . $product->mname) }}"/>
	@endif
	<title>
		@section('title')
				...
				<div class="inner group">
					...
					<div id="logo" class="group">
						<a href="{{route('home')}}" title="">
							<img src="{{ asset('assets/images/pivn-logo.png') }}" height="110" alt=""/>
						</a>
					</div>
					<!-- END LOGO -->
					<!-- START LINKSBAR -->
