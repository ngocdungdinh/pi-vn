...
@section('content')
	<h3>
    	<span class="glyphicon glyphicon-th-large"></span> Sửa chuyên mục 
    	<a href="{{ route('createcategory/cart') }}" class="btn btn-info btn-xs">Tạo chuyên mục</a>
    </h3>
  	<form method="post" action="" autocomplete="off" class="form-horizontal" role="form">
		...
		<div class="col-lg-2">
			...
			@if ( Sentry::getUser()->hasAnyAccess(['cart','cart.deletecategory']) )
				<a onclick="confirmDelete(this); return false;" href="{{ route('deletecategory/cart', $category->id) }}" class="btn btn-danger btn-xs">@lang('button.delete')</a>
			@endif
		</div>
	</form>
@stop
